    ...
    if breakdown:
        for item in breakdown:
            ...
            ">
                ...
                <span style="color: #888; flex: 1;">Conf: <b style="color:{cor_conformidade(item['conf'])};">{formatar_percentual(item['conf'])}</b></span>
            </div>
            """
    ...
    ">
        ...
        <div style="display: flex; justify-content: space-between; text-align: left;">
            ...
            <div style="flex: 1.2; padding-right: 5px;">
                ...
                <p style="font-size: 22px; font-weight: bold; margin-top: 2px; color: {cor_conformidade(conformidade)};">{formatar_percentual(conformidade)}</p>
            </div>
            <div style="flex: 1;">
...
def formatar_percentual(valor):
    ...
    if round(valor, 1) == 100.0:
        return "100%"
    if round(valor, 1) == 0.0:
        return "0%"
    return f"{valor:.1f}%"

def cor_conformidade(valor):
    """Retorna uma cor (verde/laranja/vermelho) de acordo com o nível de conformidade,
    para destacar visualmente indicadores ruins."""
    if pd.isna(valor):
        return "#666"
    if valor >= 80:
        return "#2E7D32"
